...
texture2D g_CurrentBuffer;

float fWeight[7] =
{
	0.4868, 0.7271, 0.9231, 1,
	0.278, 0.1353, 0.0561
...
PS_OUT PS_XBLUR(PS_IN In)
{
	...
	{
		uv = t + float2(tu * i, 0);
		Out.vColor += (fWeight[3 + i]) * g_CurrentBuffer.Sample(LinearSampler, uv);
	}
...
PS_OUT PS_YBLUR(PS_IN In)
{
	...
	{
		uv = t + float2(0, tv * i);
		Out.vColor += fWeight[3 + i] * g_CurrentBuffer.Sample(LinearSampler, tv);
	}
...
technique11 DefaultTechnique
{
	pass Pass0
{
	SetRasterizerState(RS_ZENABLE_FALSE);
